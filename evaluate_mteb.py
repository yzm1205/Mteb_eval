...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate LLMs on MTEB benchmark tasks."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Name of the model to evaluate (e.g., "openelm", "olmo")',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        help='Device for model (e.g., "cuda", "cpu", "auto")',
    )
    ...
    tasks = mteb.get_tasks(
        task_types=desired_task_types, languages=["eng"]
    )  # Example single task
    ...
    results = evaluation.run(
        ...
        output_folder=str(
            output_dir / model_meta.name.replace("/", "_")
        ),  # Use model_meta.name
    )
    ...
    try:
                    ...
                    for metric_name, metric_value in split_metrics.items():
                        if isinstance(metric_value, (int, float)):
                            ...
                            for (
                                ...
                                print(
                                    f"      {sub_metric_name}: {sub_metric_value:.4f}"
                                )
                        else:
        ...
        # Save to JSON
        result_path = (
            output_dir / f"mteb_results_{model_meta.name.replace('/', '_')}.json"
        )
